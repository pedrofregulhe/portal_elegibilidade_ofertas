...
def carregar_base():
    ...
    try:
        ...
        # Cria coluna CNPJ_Limpo: só números e tira os zeros à esquerda (resolve problema do Excel)
        if 'Account.CNPJ__c' in df.columns:
            df['CNPJ_Limpo'] = df['Account.CNPJ__c'].astype(str).str.replace(r'[^0-9]', '', regex=True).str.lstrip('0')
        else:
            df['CNPJ_Limpo'] = ""
    ...
    with col_resultado:
        if buscar:
            if termo_input == "":
                ...
                # Limpa o input do usuário: deixa só números e tira zeros à esquerda
                termo_limpo = re.sub(r'[^0-9]', '', termo_input).lstrip('0')
                
                # Procura no Contrato (exato) OU no CNPJ Limpo
                ...
                if clientes_encontrados.empty:
                    ...
                    for index, row in clientes_encontrados.iterrows():
                        ...
                        if str(documento) == 'nan': documento = 'Doc não informado'
